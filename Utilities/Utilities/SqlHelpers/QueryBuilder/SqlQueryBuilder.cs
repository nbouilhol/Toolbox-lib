...
namespace Utilities.QueryBuilder
{
    ...
    {
        private readonly SqlJoinBuilder _sqlJoinBuilder;
        private readonly SqlQueryRequest _sqlQueryRequest;
        private readonly SqlSelectBuilder _sqlSelectBuilder;
        private readonly SqlWhereBuilder _sqlWhereBuilder;
        ...
        private void BuildWhere()
        {
            if (_sqlQueryRequest.Wheres != null)
            {
                if (_sqlQueryRequest.Wheres.TrimEnd().EndsWith("AND"))
                    _sqlQueryRequest.Wheres = _sqlQueryRequest.Wheres.Substring(0,
                        _sqlQueryRequest.Wheres.LastIndexOf("AND"));
                if (_sqlQueryRequest.Wheres.TrimEnd().EndsWith("OR"))
                    _sqlQueryRequest.Wheres = _sqlQueryRequest.Wheres.Substring(0,
                        _sqlQueryRequest.Wheres.LastIndexOf("OR"));
            }
        ...
        private void BuildFrom()
        {
            ...
            if (string.IsNullOrEmpty(_sqlQueryRequest.FromSubQuery) && string.IsNullOrEmpty(_sqlQueryRequest.From))
                return;
        ...
        private void BuildMax()
        {
            ...
            if (!string.IsNullOrEmpty(_sqlQueryRequest.MaxAlias))
                queryMax += string.Format(" AS {0} ", _sqlQueryRequest.MaxAlias);
        ...
        private void BuildMin()
        {
            ...
            if (!string.IsNullOrEmpty(_sqlQueryRequest.MinAlias))
                queryMin += string.Format(" AS {0} ", _sqlQueryRequest.MinAlias);
        ...
        private void BuildSum()
        {
            ...
            if (!string.IsNullOrEmpty(_sqlQueryRequest.SumAlias))
                querySum += string.Format(" AS {0} ", _sqlQueryRequest.SumAlias);
        ...
        private void BuildAvg()
        {
            ...
            if (!string.IsNullOrEmpty(_sqlQueryRequest.AvgAlias))
                queryAvg += string.Format(" AS {0} ", _sqlQueryRequest.AvgAlias);
        ...
        private void BuildCount()
        {
            ...
            if (!string.IsNullOrEmpty(_sqlQueryRequest.CountAlias))
                queryCount += string.Format(" AS {0} ", _sqlQueryRequest.CountAlias);
        ...
        private void BuildSelectedColumns()
        {
            if ((_sqlQueryRequest.Columns == null || _sqlQueryRequest.Columns.Count == 0)
                &&
                (string.IsNullOrEmpty(_sqlQueryRequest.MaxColumnName) &&
                 string.IsNullOrEmpty(_sqlQueryRequest.MinColumnName)
                 && string.IsNullOrEmpty(_sqlQueryRequest.SumColumnName) &&
                 string.IsNullOrEmpty(_sqlQueryRequest.CountColumnName))
                )
                query.Append(" * ");
            if (_sqlQueryRequest.Columns != null && _sqlQueryRequest.Columns.Count > 0)
        ...
        {
            _sqlSelectBuilder.OrderBy(null, new[] {sortColumnName}, sortOrder);
            return this;
        }
