...
namespace Utilities.SqlHelpers
{
    public class QueryBulk : IDisposable
    {
        ...
        private static DataTable CreateDataTable<T>(IList<T> list, SqlBulkCopy bulkCopy)
        {
            ...
            PropertyDescriptor[] props = TypeDescriptor.GetProperties(typeof(T)).Cast<PropertyDescriptor>().Where(propertyInfo => propertyInfo.PropertyType.Namespace.Equals("System")).ToArray();
