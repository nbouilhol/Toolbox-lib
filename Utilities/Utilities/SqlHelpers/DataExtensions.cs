...
namespace Utilities.SqlHelpers
{
    public static class DataExtensions
    {
        ...
        public static DataTable AddToDataTable<TParent, T>(this DataTable table, string property,
            IEnumerable<TParent> data, Func<TParent, T> func, params string[] propertiesNameToTake)
            where T : class
        {
            ...
            foreach (PropertyInfo propertyInfo in propertiesInfos)
                table.Columns.Add(string.Format(FormatNameForAddedProperty, property, propertyInfo.Name),
            ...
            foreach (TParent item in data.WhereNotNull())
            {
                ...
                IEnumerable<KeyValuePair<string, Func<T, object>>> accessors = FilterAccessors(mappedProperties,
                    propertiesNameToTake);

                foreach (var accessor in accessors)
                {
                    T value = func(item);
                    row[string.Format(FormatNameForAddedProperty, property, accessor.Key)] = value != null
                        ? accessor.Value(value) ?? DBNull.Value
                        : DBNull.Value;
                }
            }
        ...
        private static DataTable GetDataTable<T>(this IEnumerable<T> data,
            IEnumerable<PropertyDescriptor> mappedProperties, params string[] propertiesNameToTake)
        {
        ...
        private static DataTable GetDataTable<T>(this IEnumerable<T> data, PropertiesDescriptor<T> mappedProperties,
        ...
        public static DataTable ToDataTable<T>(this IEnumerable<T> data, params string[] propertiesNameToTake)
            where T : class
        {
            ...
            params string[] propertiesNameToTake) where T : class
        {
            IEnumerable<PropertyDescriptor> properties =
                typeof (T).GetPropertiesInfoWithInterfaces().Cast<PropertyDescriptor>().Where(expression);
            return GetDataTable(data, properties, propertiesNameToTake);
        }

        public static DataTable ToDataTable<T>(this IEnumerable<T> data, IEnumerable<PropertyDescriptor> properties,
            params string[] propertiesNameToTake) where T : class
        {
            return GetDataTable(data, properties, propertiesNameToTake);
        }

        public static DataTable ToDataTable<T>(this IEnumerable<T> data, PropertiesDescriptor<T> properties,
            params string[] propertiesNameToTake) where T : class
        {
            return GetDataTable(data, properties, propertiesNameToTake);
        }
        ...
        private static IEnumerable<PropertyDescriptor> FilterPropertiesDescriptor(
            IEnumerable<PropertyDescriptor> mappedProperties, params string[] propertiesNameToTake)
        {
            return propertiesNameToTake != null && propertiesNameToTake.Length > 0
