...
namespace Utilities.Helpers
{
    ...
    {
        private readonly ConcurrentBag<ComposablePartDefinition> _parts = new ConcurrentBag<ComposablePartDefinition>();
        ...
        public void RegisterType<TImplementation, TContract>()
        {
            ComposablePartDefinition part =
                ReflectionModelServices.CreatePartDefinition(new Lazy<Type>(() => typeof (TImplementation)), false,
                    new Lazy<IEnumerable<ImportDefinition>>(() => GetImportDefinitions(typeof (TImplementation))),
                    new Lazy<IEnumerable<ExportDefinition>>(
                        () => GetExportDefinitions(typeof (TImplementation), typeof (TContract))),
                    new Lazy<IDictionary<string, object>>(() => new Dictionary<string, object>()), null);
        ...
        {
            ComposablePartDefinition part =
                ReflectionModelServices.CreatePartDefinition(new Lazy<Type>(() => concreteType), false,
                    new Lazy<IEnumerable<ImportDefinition>>(() => GetImportDefinitions(concreteType)),
                    new Lazy<IEnumerable<ExportDefinition>>(() => GetExportDefinitions(concreteType, interfaceType)),
                    new Lazy<IDictionary<string, object>>(() => new Dictionary<string, object>()), null);
        ...
        private ImportDefinition[] GetImportDefinitions(Type implementationType)
        {
            ...
            foreach (ParameterInfo param in constructors.GetParameters())
            {
                ...
                Type importType = cardinality == ImportCardinality.ZeroOrMore
                    ? GetCollectionContractType(param.ParameterType)
                    : param.ParameterType;

                imports.Add(ReflectionModelServices.CreateImportDefinition(new Lazy<ParameterInfo>(() => param),
                    AttributedModelServices.GetContractName(importType),
                    AttributedModelServices.GetTypeIdentity(importType), Enumerable.Empty<KeyValuePair<string, Type>>(),
                    cardinality, CreationPolicy.Any, null));
            }
